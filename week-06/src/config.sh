...
export CLIENT=ChimeDemo
export PASS=1

./openvpn-install.sh

chown ec2-user /root/ChimeDemo.ovpn
cp /root/ChimeDemo.ovpn /tmp/
cd /tmp
...
PrivateSBCAddress=$( aws ssm get-parameter --name /asterisk/privateSBCAddress --region $REGION | jq -r '.Parameter.Value' )
...
match=$PrivateSBCAddress

[VoiceConnector]
type=aor
contact=sip:$PrivateSBCAddress
