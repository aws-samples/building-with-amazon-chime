...
export class AsteriskFaxServerStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string) {
    ...
    const incomingFaxes = new s3.Bucket(this, 'IncomingFaxes', {
    ...
    const textractRole = new iam.Role(this, 'textractLambdaRole', {
      assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
      inlinePolicies: {
        ['chimePolicy']: new iam.PolicyDocument( { statements: [new iam.PolicyStatement({
          resources: ['*'],
    ...
    const textractLambda = new lambda.Function(this, 'textractLambda', {
      code: lambda.Code.fromAsset("src", {exclude: ["**", "!textract.py"]}),
    ...
    textractLambda.addEventSource(new S3EventSource(incomingFaxes, {
      events: [ s3.EventType.OBJECT_CREATED]
    }));

    incomingFaxes.grantReadWrite(textractLambda)
    ...
    const incomingFaxBucketParameter = new ssm.StringParameter(this, 'incomingFaxBucket', {
      parameterName: '/asterisk/incomingFaxBucket',
      stringValue: incomingFaxes.bucketName
    })
    ...
    incomingFaxBucketParameter.grantRead(role);
    ...
    new cdk.CfnOutput(this, 'S3 Bucket', { value: incomingFaxes.bucketName} )
    new cdk.CfnOutput(this, 'Textract Lambda', { value: textractLambda.functionName})
  }
}