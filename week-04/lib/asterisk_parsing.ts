...
export class AsteriskParsing extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props: StackProps) {
    ...
    const processCDRsLambda = new lambda.Function(this, 'process_cdrs', {
      code: lambda.Code.fromAsset("src", {exclude: ["**", "!process_cdrs.js"]}),
      handler: 'process_cdrs.handler',
    ...
    const voiceConnectorResource = new CustomResource(this, 'voiceConnectorResource', { 
      ...
      properties: { 'region': this.region,
                    'eip': eip.ref,
                    'cdr_bucket': cdrBucket.bucketName }
    })
    ...
    const voiceConnectorLogGroupName = '/aws/ChimeVoiceConnectorSipMessages/' + voiceConnectorId
    const sip_logs = logs.LogGroup.fromLogGroupName(this, 'VoiceConnectorLog', voiceConnectorLogGroupName )
    
    const processRole = new iam.Role(this, 'processRole', {
      assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com')}
    )
    
    const processRolePolicy = new iam.PolicyStatement({
      resources: [ sip_logs.logGroupArn ],
      actions: ['cloudwatatch:*',
                'logs:*']}) 
    processRole.addManagedPolicy(iam.ManagedPolicy.fromAwsManagedPolicyName("service-role/AWSLambdaBasicExecutionRole"))
    processRole.addToPolicy(processRolePolicy)

    const processLogsLambda = new lambda.Function(this, 'processLogsLambda', {
      code: lambda.Code.fromAsset("src", {exclude: ["**", "!process_logs.py"]}),
    ...
    sip_logs.addSubscriptionFilter('SubLogs', {
      destination: new LambdaDestination(processLogsLambda),
      filterPattern: logs.FilterPattern.allTerms("INVITE sip")
    })

    processLogsLambda.addPermission('logsPermissions', {
      principal: new iam.ServicePrincipal('logs.us-east-1.amazonaws.com'),
      action: 'lambda:invokeFunction',
      sourceArn: sip_logs.logGroupArn
    })
